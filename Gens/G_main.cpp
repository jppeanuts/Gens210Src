...
int Set_Current_State(HWND hWnd, int Num)
{
	...
	{
		fclose(f);
		MESSAGE_NUM_L("SLOT %d [EMPTY]", "SLOT %d [OCCUPIED]", Current_State, 1500)
	}
	else
...
int Change_YM2612_Improv(HWND hWnd)
{
	...
	{
		YM2612_Improv = 1;
		MESSAGE_L("High Quality YM2612 emulation", "High Quality YM2612 emulation (slower)", 1000)
	}
		...
		case WM_COMMAND:
			...
			else switch(LOWORD(wParam))
			{
				...
				case ID_FILES_SAVESTATEAS:
					if (Check_If_Kaillera_Running()) return 0;
					Change_File_S(Str_Tmp, State_Dir, "Load state", "State Files\0*.gs*\0All Files\0*.*\0\0", "");
					Save_State(Str_Tmp);
					return 0;
